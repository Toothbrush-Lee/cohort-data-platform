...
export default function StudyVisitsPage() {
  ...
  const handleSubmit = async () => {
    try {
      await visitsApi.create(studyId, {
  ...
  const handleBatchSubmit = async () => {
    ...
    try {
      const result = await visitsApi.batchCreate(studyId, {
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="container mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold">随访记录管理</h1>
            <p className="text-sm text-gray-500 mt-1">研究 ID: {studyId}</p>
          </div>
          <div className="flex gap-2">
            <Link href={`/studies/${studyId}/visits/import`}>
          ...
          <CardContent>
            {loading ? (
              ...
              <div className="text-center py-8 text-gray-500">
                暂无数据
                <div className="mt-2">
                  <Button variant="link" onClick={() => setDialogOpen(true)}>去创建第一个随访</Button>
                </div>
              </div>
            ) : (
              <Table>
                  ...
                  {visits.map((visit) => {
                    ...
                    return (
                      <TableRow key={visit.id}>
                        ...
                        <TableCell>
                          <div className="flex gap-2">
                            <Link href={`/studies/${studyId}/upload?visit_id=${visit.id}`}>
                              <Button variant="ghost" size="sm">上传</Button>
                            </Link>
                            <Link href={`/studies/${studyId}/enter?visit_id=${visit.id}`}>
