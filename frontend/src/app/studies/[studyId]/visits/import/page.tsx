  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000/api/v1'}/visits/import/excel?study_id=${studyId}`,
        {
          method: 'POST',
          headers: {
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="container mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold">批量导入随访记录</h1>
            <p className="text-sm text-gray-500 mt-1">研究 ID: {studyId}</p>
          </div>
          <div className="flex gap-2">
            <Link href={`/studies/${studyId}/visits`}>
              <Button variant="outline">返回随访列表</Button>
            </Link>
          </div>
        </div>
              ...
              <div className="bg-blue-50 rounded-lg p-4">
                ...
                <table className="w-full text-sm text-blue-800">
                  ...
                  <tbody>
                    ...
                    <tr>
                      <td className="py-2 font-medium">visit_name</td>
                      <td>随访类型（Baseline/V1/V3/V6/V12/Other）</td>
                      <td>是</td>
                    </tr>
                    <tr>
                      <td className="py-2 font-medium">visit_date</td>
                      <td>随访日期（YYYY-MM-DD）</td>
                      <td>是</td>
                    </tr>
                    <tr>
                      ...
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="border-b">
                            ...
                            <th className="text-left">随访类型</th>
                          </tr>
                        </thead>
                        <tbody>
                          ...
                          {importResult.success.length > 10 && (
                            <tr>
                              <td colSpan={3} className="text-gray-500 italic py-1">
